...
#if defined(NDEBUG)
# error "Royalbitcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Royalbitcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s RBTC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	//classic RBTC Subsidy
	if(nHeight < RRBTC_HARD_FORK_HEIGHT)
	{
	...
	{
		int iHeightOffset = nHeight - RRBTC_HARD_FORK_HEIGHT;
		int halvings = iHeightOffset / consensusParams.nSubsidyHalvingInterval;
		// Force block reward to zero when right shift is undefined.
		if (halvings >= 64)
			return 0;

		//RRBTC 初始为  5000 RRBTC
		nSubsidy = 5000 * COIN;
		// Subsidy is cut in half every 210,000 blocks which will occur approximately every 4 years.
		nSubsidy >>= halvings;
	}
...
void ThreadScriptCheck() {
    RenameThread("royalbitcoin-scriptch");
...
{
    LOCK(cs_main);
    int nHeightNext = pindexPrev->nHeight + 1;
    int32_t nVersion = (nHeightNext >= RRBTC_HARD_FORK_HEIGHT? VERSIONBITS_TOP_BITS:VERSIONBITS_TOP_BITS_FOR_CLASSIC);
...
public:
    ...
    {
        return ((pindex->nVersion & VERSIONBITS_TOP_MASK) == (pindex->nHeight>= RRBTC_HARD_FORK_HEIGHT? VERSIONBITS_TOP_BITS:VERSIONBITS_TOP_BITS_FOR_CLASSIC)) &&
...
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    ...
    do {
        ...
        bool fInitialDownload;
        {
            ...
            //区块同步阀值: 最大值为 (RRBTCHeight-1)
            int iMaxSyncHeight = gArgs.GetArg("-maxsyncheight", MAX_CLASSIC_SYNC_HEIGHT);

            //synctype=0 仅同步经典RBTC区块，则需要做同步高度阀值的校验
            if( 0 == gArgs.GetArg("-synctype", DEFAULT_SYNC_TYPE) &&
            		(chainActive.Tip() && chainActive.Tip()->nHeight >= std::min(iMaxSyncHeight, MAX_CLASSIC_SYNC_HEIGHT) ))
            {
            	LogPrintf("%s %d|synctype=0,block sync, meet the maxHeight, exit|curr best height=%d|iMaxSyncHeight=%d|MAX_CLASSIC_SYNC_HEIGHT=%d\n", __FILE__, __LINE__, chainActive.Tip()->nHeight, iMaxSyncHeight, MAX_CLASSIC_SYNC_HEIGHT);
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    //seed node has height limit , must less than RRBTC_HARD_FORK_HEIGHT
    if (gArgs.GetArg("-hardforkseed", 0))
    {
    	if(nHeight >= consensusParams.RRBTCHeight )
    		return state.Invalid(false, REJECT_INVALID, "seed height limit", "seed height limit, must less than RRBTC_HARD_FORK_HEIGHT ");
    }
    ...
    if((block.nVersion < 2 && nHeight >= consensusParams.BIP34Height) ||
       ...
       ( block.nVersion < VERSIONBITS_TOP_BITS && nHeight >= consensusParams.RRBTCHeight ) )
