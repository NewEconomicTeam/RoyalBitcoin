...
UniValue getblockhashes(const JSONRPCRequest& request)
{
    ...
    while (block &&
           ...
           block->GetBlockTime() > (int64_t)low &&
           block->GetBlockTime() < (int64_t)high)
    {
        //std::cout << "block->GetBlockTime() =>" << block->GetBlockTime() << std::endl;
        blockHashes.push_back(std::make_pair(block->GetBlockHash(), (unsigned int)block->GetBlockTime()));
        block = block->pprev;
    }

    for (std::vector<std::pair<uint256, unsigned int> >::const_reverse_iterator it=blockHashes.rbegin();
         it!=blockHashes.rend();
         it++)
    {
        if (fLogicalTS) {
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of royalbitcoin addresses\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "getblockhashes",         &getblockhashes,         true,  {"michael"} },
