             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            ...
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CRoyalbitcoinAddress(strAddress).Get()].name;
        }
        else if (strType == "purpose")
        {
            std::string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CRoyalbitcoinAddress(strAddress).Get()].purpose;
        }
        else if (strType == "tx")
        {
            uint256 hash;
        ...
        else if (strType == "destdata")
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CRoyalbitcoinAddress(strAddress).Get(), strKey, strValue))
            {
