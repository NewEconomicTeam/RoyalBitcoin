    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            if (InsecureRandRange(5) == 0 || coin.IsSpent()) {
                Coin royalbitcoin;
                royalbitcoin.out.nValue = InsecureRand32();
                royalbitcoin.nHeight = 1;
                if (InsecureRandRange(16) == 0 && coin.IsSpent()) {
                    royalbitcoin.out.scriptPubKey.assign(1 + InsecureRandBits(6), OP_RETURN);
                    BOOST_CHECK(royalbitcoin.out.scriptPubKey.IsUnspendable());
                    added_an_unspendable_entry = true;
                } else {
                    royalbitcoin.out.scriptPubKey.assign(InsecureRandBits(6), 0); // Random sizes so we can test memory usage accounting
                    (coin.IsSpent() ? added_an_entry : updated_an_entry) = true;
                    coin = royalbitcoin;
                }
                stack.back()->AddCoin(COutPoint(txid, 0), std::move(royalbitcoin), !coin.IsSpent() || InsecureRand32() & 1);
            } else {
