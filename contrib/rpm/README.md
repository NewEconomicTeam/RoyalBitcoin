...
The RPM spec file provided here is for Royalbitcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://royalbitcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Royalbitcoin git repository](https://github.com/royalbitcoin/royalbitcoin) but are not part of
...
Currently only `Source10` is used, for the example `royalbitcoin.conf` file.
...
in the royalbitcoin git repository. At present only an SVG version of the Royalbitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the royalbitcoin
...
needed when 0.12.1 is released, a proper fix is already in the Royalbitcoin git
master branch.
...
idea to build Royalbitcoin Core against a static version of that library compiled
...
The `royalbitcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `royalbitcoin-server` package. At this point, most RPM based
...
The `royalbitcoin-server` package should have SELinux support. How to properly do
...
Royalbitcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/royalbitcoin/). He has been
packaging Royalbitcoin for Fedora at least since 2012.
...
4. I renamed the `royalbitcoin` package that contains the Qt GUI to `royalbitcoin-core` as
that appears to be how the general population refers to it, in contrast to
`royalbitcoin-xt` or `royalbitcoin-classic`. I wanted to make sure the general population
