...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Royalbitcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Royalbitcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Royalbitcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Royalbitcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Royalbitcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Royalbitcoin 0.8 datadir."
    ;;
esac
