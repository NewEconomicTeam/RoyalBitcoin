...
###   royalbitcoincore.org and royalbitcoin.org and compares them.
...
WORKINGDIR="/tmp/royalbitcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://royalbitcoincore.org"
HOST2="https://royalbitcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="royalbitcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'royalbitcoin-',
...
if [ $? -ne 0 ]; then
   echo "royalbitcoin.org failed to provide signature file, but royalbitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "royalbitcoin.org and royalbitcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Royalbitcoin Core binary release signing key installed?"
   fi
